//! Backend wrappers around [`jrpxy_body::writer::chunked`]. Each state
...
/// Finish (terminal) state. Yields a [`BackendWriter`] ready for the
/// next request once the underlying writer is flushed.
#[derive(Debug)]
pub struct Finish<I> {
...
/// Abort (terminal) state. Consumes the writer; the connection is not
/// reusable.
#[derive(Debug)]
pub struct Abort<I> {
