...
pub enum BodyError {
    ...
    IncompleteChunkFooter { expected: u64, actual: u64 },
    #[error("Finish expected {expected} bytes, but only wrote {actual} bytes")]
    IncompleteFinish { expected: u64, actual: u64 },
    #[error("Finish completed writing but was not flushed")]
    UnflushedFinish,
    #[error("Abort was not flushed")]
    UnflushedAbort,
